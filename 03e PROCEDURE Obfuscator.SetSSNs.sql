...
CREATE OR ALTER PROCEDURE Obfuscator.SetSSNs
...
UPDATE st SET
	st.' + QUOTENAME(@SsnColumn) + ' = ' + CASE WHEN @CharMaxLength = -1 THEN 'fake.SSN' ELSE 'LEFT(fake.SSN, ' + CAST(@CharMaxLength AS NVARCHAR(MAX)) + ')' END + '
FROM SourceTable st
JOIN #FakeData fake
